  ...
  methods: {
    ...
    getCodec(codecKey) {
      const codecs = {
        ...
        octal: {
          encode: input => Array.from(input).map(c => c.charCodeAt(0).toString(8).padStart(3, '0')).join(' '),
          decode: input => {
            return input.split(' ').map(oct => {
              if (!oct.trim()) return ''
              const num = parseInt(oct, 8)
              if (isNaN(num)) {
                throw new Error('无效的八进制数据')
              }
              return String.fromCharCode(num)
            }).join('')
          }
        ...
        base58: {
          encode: this.base58Encode,
          decode: this.base58Decode
        },
        md5: {
    ...
    morseEncode(input) {
      ...
      return words.map(word => {
        return word.toUpperCase().split('').map(c => morseCode[c] || c).join(' ')
      }).join(' / ')
    },
    morseDecode(input) {
      const morseCode = {
    ...
    base32Decode(input) {
      ...
      let bits = 0, value = 0, result = ''
      for (let i = 0; i < input.length; i++) {
        const index = base32Alphabet.indexOf(input[i])
        if (index === -1) {
    ...
    base58Decode(input) {
      ...
      let i, j, bytes = [0]
      for (i = 0; i < input.length; i++) {
        const index = base58Alphabet.indexOf(input[i])
        if (index === -1) {
