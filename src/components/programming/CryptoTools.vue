  ...
  methods: {
    ...
    validateAesInput() {
      ...
      if (keyLength !== expectedLength) {
        throw new Error(`密钥长度必须为 ${expectedLength} 字节，当前为 ${keyLength} 字节`)
      }
      ...
      if (this.aesOptions.mode !== 'ECB' && this.aesOptions.iv.length !== 16) {
        throw new Error(`初始化向量必须为16字节，当前为 ${this.aesOptions.iv.length} 字节`)
      }
    },
      ...
      const cryptos = {
        ...
        rsa: {
          encrypt: (input, key) => {
            ...
            return forge.util.encode64(publicKey.encrypt(input, 'RSA-OAEP', {
              md: forge.md.sha256.create()
            }))
          },
          decrypt: (input, key) => {
            const privateKey = forge.pki.privateKeyFromPem(key)
            return privateKey.decrypt(forge.util.decode64(input), 'RSA-OAEP', {
              md: forge.md.sha256.create()
            })
          }
        },
        ...
        pbkdf2: {
          encrypt: (input, key) => {
            // PBKDF2 密钥派生 - key作为salt使用
            const salt = CryptoJS.enc.Utf8.parse(key || input)
            const derivedKey = CryptoJS.PBKDF2(input, salt, {
              keySize: 256/32,
              iterations: 10000
            })
            return `Salt: ${salt.toString(CryptoJS.enc.Hex)}\nDerived Key: ${derivedKey.toString(CryptoJS.enc.Hex)}`
          },
          decrypt: () => 'PBKDF2 不支持解密'
        }
      }
